#!/usr/bin/env Rscript
#
# VFDB.R  —  Shell-gene cluster virulence analysis (VFDB track)
#
# Split out of the original virulence_analysis.R (which ran the VFDB and
# VirulenceFinder tracks back-to-back). This file keeps only the VFDB half;
# see VirulenceFinder.R for the VF half. Both files duplicate the shared
...
#
# Maps VFDB to shell-gene clusters, detects temporal trends, selects the
# cluster with strongest VF signal, and performs clinical enrichment +
# figures.
#
# Usage:
#   TARGET_ST=ST10 Rscript scripts/Virulence/VFDB.R
#
...
# -------------------------------------------------------------------
# 2. Analysis function (VFDB track)
# -------------------------------------------------------------------
...
# 3. VFDB track
# -------------------------------------------------------------------
ts("Reading VFDB summary...")
vfdb_file <- config$st_vfdb_summary()
if (file.exists(vfdb_file)) {
  raw <- read_tsv(vfdb_file, show_col_types = FALSE, col_types = cols(.default = "c"))
  genes <- setdiff(colnames(raw), c("#FILE", "NUM_FOUND"))
  extract_id <- function(f) gsub("_vfdb\\.tsv$", "", basename(f))
  df <- raw %>% mutate(genome_id = extract_id(.data[["#FILE"]]))
  ...
  df <- common(df)
  if (nrow(df) > 50) {
    res_vfdb <- analyze_vf_track("VFDB", df)
  } else { ts("VFDB: too few genomes"); res_vfdb <- NULL }
} else { ts("VFDB file not found") }

# -------------------------------------------------------------------
# 4. Summary
# -------------------------------------------------------------------
cat("\n========================================\n")
cat("  VFDB VIRULENCE ANALYSIS COMPLETE -", st, "\n")
cat("========================================\n")
if (exists("res_vfdb") && !is.null(res_vfdb)) {
  cat("VFDB: target =", res_vfdb$target, "\n")
}
cat("Output:", OUT_DIR, "\n")
ts("DONE")
